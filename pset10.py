...
class Hand(object):
    ...
    def containsLetters(self, letters):
        ...
        for letter in letters:
          if letter not in self.handDict.keys():
            return False
          else:
            temp_dict[letter] = temp_dict.get(letter, 0) + 1
            if temp_dict[letter] > self.handDict[letter]:
              return False
        return True
    def isEmpty(self):
        """
    ...
    def __eq__(self, other):
        ...
        for letter in self.handDict.keys():
          try:
            if self.handDict[letter] != other.hadDict[letter]:
              return False
          except KeyError:
            return False
        return True
    ...
    def pickBestWord(self, wordlist):
        ...
        for word in wordlist.getList():
          ...
          if self.hand.containsLetters(word):
            if getWordScore(word) > hi_pt:
              ans = word
              hi_pt = getWordScore(word)
        if ans == 'NONE':
          print("No answer.")
          return '.'
